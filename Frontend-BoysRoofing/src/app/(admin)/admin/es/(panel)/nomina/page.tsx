...
export default function NominaES() {
  ...
  function createPeriod(e: React.FormEvent) {
    ...
    apiFetch("/payroll/periods", {
      ...
      body: JSON.stringify({
        startDate,
        endDate,
        label: label.trim() || defaultPayrollPeriodLabel(startDate, endDate, "es-ES"),
      }),
    })
      .then((r) => {
  ...
  function exportPayrollCsv() {
    const csv = buildPayrollReportCsv(periods, locale, {
      monthKeyHeader: "Clave mes (AAAA-MM)",
      monthLabelHeader: "Mes",
      totalPaidHeader: "Total pagado",
      periodCountHeader: "Periodos de nómina",
      idHeader: "ID periodo",
      nameHeader: "Nombre",
      startHeader: "Fecha inicio",
      endHeader: "Fecha fin",
      statusHeader: "Estado",
      amountPaidHeader: "Importe pagado",
    });
    const stamp = new Date().toISOString().slice(0, 10);
    downloadPayrollReportCsv(`nomina-informe-${stamp}.csv`, csv);
  }
  ...
  return (
    <div className="space-y-8 p-6 text-white">
              ...
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
                  ...
                  {rangeOpen && (
                    <div className="absolute left-0 top-full z-10 mt-1 rounded-lg border border-white/10 bg-br-carbon p-3 shadow-xl">
                      <PayrollPeriodRangeCalendar
                        ...
                        helperText="Elige inicio y fin. Usa las flechas para cambiar de mes (el rango puede cruzar dos meses)."
                ...
                <div className="sm:col-span-2">
                  ...
                  <input
                    ...
                    placeholder="ej. Semana 3 – cuadrilla A (si vacío, se usan las fechas)"
      ...
      {monthlyReport.length > 0 && (
        <div className="admin-card-glow overflow-hidden animate-fade-up">
          <div className="flex flex-col gap-3 border-b border-white/10 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <h2 className="text-lg font-semibold text-br-pearl">Totales por mes (pagado)</h2>
            <button
              type="button"
          ...
          <p className="px-5 pt-3 text-xs text-br-white/50">
            Suma del importe “Pagado” de cada periodo, agrupada por el mes de inicio del periodo.
          </p>
            ...
            {monthlyReport.map((row) => (
              ...
              >
                ...
                <div className="mt-1 text-xs text-br-white/45">
                  {row.periodCount} periodo{row.periodCount !== 1 ? "s" : ""}
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="space-y-6">
        <h2 className="text-lg font-semibold text-br-pearl">Historial de nóminas</h2>
        {periods.length === 0 ? (
            ...
            >
              <div className="border-b border-white/10 bg-gradient-to-r from-br-carbon/90 via-br-smoke/50 to-br-carbon/80 px-5 py-3.5">
                ...
                <p className="mt-0.5 text-xs text-br-white/45">
                  {group.periods.length} periodo{group.periods.length !== 1 ? "s" : ""}
                </p>
              </div>
                      ...
                      >
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-white/5 pt-3 text-sm">
                          <span className="tabular-nums font-medium text-br-white/85">
                            Pagado: ${Number(totalPaid).toFixed(2)}
                          </span>
