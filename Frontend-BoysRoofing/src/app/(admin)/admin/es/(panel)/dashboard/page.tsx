...
export default function AdminESDashboard() {
    ...
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center gap-4">
        <div
          ...
          aria-hidden
        />
        <p className="text-sm text-br-white/55 tracking-wide">Cargando panel…</p>
      </div>
    );
  }
  ...
  const breakdownRows = [
    { key: "inReview", label: "En revisión", value: stats.inReview, bar: "bg-sky-400" },
    { key: "sent", label: "Enviadas", value: stats.sent, bar: "bg-emerald-400" },
    { key: "closed", label: "Cerradas", value: stats.closed, bar: "bg-zinc-400" },
  ] as const;
    ...
    <div className="space-y-8 p-1 text-white">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-xl text-sm leading-relaxed text-br-white/60">
              Vista general de cotizaciones recibidas y su estado. Los datos reflejan todo el
      ...
      <section className="overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-b from-br-smoke/50 to-br-carbon/30 shadow-xl shadow-black/20">
        <div className="flex flex-col gap-3 border-b border-white/10 bg-gradient-to-r from-br-carbon/80 to-transparent px-5 py-4 sm:flex-row sm:items-center sm:justify-between md:px-6 md:py-5">
          <div>
            <h2 className="text-lg font-semibold text-br-pearl md:text-xl">Cotizaciones recientes</h2>
            <p className="mt-1 text-xs text-br-white/55 md:text-sm">
              Las cinco últimas entradas, ordenadas por fecha.
            </p>
          </div>
          <Link
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <p className="mt-0.5 text-xs text-br-white/45">Cotización #{q.id}</p>
                    </div>
                    <span
                      className={`shrink-0 rounded-full px-3 py-1 text-xs font-medium ${statusBadgeClass(q.status)}`}
                    >
                      {quoteStatusLabel(q.status, "es")}
                    </span>
                  </div>
                  <p className="mt-4 text-xs text-br-white/50">
                    <span className="text-br-white/40">Creada · </span>
                    {created}
                  </p>
                  <Link
