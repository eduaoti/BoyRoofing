        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-4" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="border-t border-white/10 pt-4 space-y-3">
              <p className="text-sm font-medium text-br-pearl/80">Precio total y saldo (opcional)</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs text-br-pearl/60 mb-1">Precio total del trabajo (USD)</label>
                  <input
                    type="text"
                    value={totalPrice}
                ...
                <div>
                  <label className="block text-xs text-br-pearl/60 mb-1">Referencia del trabajo (agrupar recibos)</label>
                  <input
                    type="text"
                    value={jobReference}
                    onChange={(e) => setJobReference(e.target.value)}
                    className="w-full rounded-xl bg-br-carbon/80 border border-white/10 px-4 py-2.5 text-white placeholder-br-pearl/40 focus:border-br-red-main focus:ring-1 focus:ring-br-red-main"
                    placeholder="ej. Techo Pérez 2026"
                  />
                </div>
              </div>
              <p className="text-xs text-br-pearl/50">Indica precio total y la misma referencia en los recibos del mismo trabajo para ver saldo pendiente y enviar recibo de saldo o de agradecimiento.</p>
            </div>
                ...
                <div className="min-w-0">
                  ...
                  {balance != null && balance.totalPrice > 0 && (
                    <p className="text-xs text-br-pearl/70 mt-1">
                      Total: {new Intl.NumberFormat("es-MX", { style: "currency", currency: "USD" }).format(balance.totalPrice)} · Pagado: {new Intl.NumberFormat("es-MX", { style: "currency", currency: "USD" }).format(balance.totalPaid)} · <span className={balance.balanceDue > 0 ? "text-amber-400" : "text-green-400"}>Saldo: {new Intl.NumberFormat("es-MX", { style: "currency", currency: "USD" }).format(balance.balanceDue)}</span>
                    </p>
                  )}
                </div>
